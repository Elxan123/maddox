...
new function() {
    ...
    function init() {
        ...
        $( 'h1,h2,h3,h4,h5,h6,.hp-boxes' ).widont();
    };
     ...
     */
    /**
     * EDIT (jimharedesign@googlemail.com):
     * Modified to make changes more cautious – if the modified string would
     * cause jarring edge rag it is not applied.
     */
    function widont( string ) {
    	var replaced = string.replace( Private.regexp, "&#160;$1" );
    	var splitted = replaced.split(' ');
		var lastpair = splitted[splitted.length-1];

    	if(lastpair.length<20)
        	return replaced;
        else
        	return string;
    };
